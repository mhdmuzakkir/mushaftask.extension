...
## Project Structure
    ...
    |-- css/
    |   |-- 01-variables.css      # CSS modular partials (20 files total)
    |   |-- 02-base.css
    |   |-- ... (through 20-chat.css)
    ...
    |   |-- modules/              # Modular architecture (25 files, IIFE pattern)
    ...
    |   |   |-- drive-scanner.js  # Auto-discovers mushafproject across drives & shortcuts
    ...
    |   |-- icons/                # SVG icons
    |-- docs/
    |   |-- DRIVE_SCANNER_PORTING.md
    |   |-- MUSHAFPROJECT_MIGRATION_PLAN.md
    |   |-- WORKFLOW.md
...
25. `chat.js` - Chat/messaging tab
26. `main.js` - Orchestrator
...
**Preferred**: `install.bat` installs to `%APPDATA%` without requiring Administrator privileges.
...
The UI has six tabs plus modals:
...
- **STATS** (`#statsTab`) - Activity statistics with today/week/month filters.
- **CHAT** (`#chatTab`) - Chat/messaging functionality.
...
14. **CSS is modular** - As of v2.5.5, styles are split into 20 partial files (`01-variables.css` through `20-chat.css`). The legacy monolithic `styles.css` may still exist but should not be edited. Edit the partials instead.

---

## Changelog (Session: June 6, 2026)

### Mushaf Task Manager - v2.2.0 -> v2.5.5
- **CSS Modularization**: Split monolithic `styles.css` into 20 partial files (`01-variables.css` through `20-chat.css`).
- **Drive Missing Modal**: Added recovery modal when saved folders are on a disconnected drive.
- **Updater Node.js Guard**: Fixed updater to properly guard Node.js `child_process` usage.
- **Runtime Version Reading**: `updater.js` now reads `CURRENT_VERSION` from `version.json` at runtime instead of hardcoding.
- **Chat Tab**: Added chat/messaging functionality with new `chat.js` module (25th module).
- **Auto-Fit Artboard**: Automatically executes Ctrl+0 (Fit in Window) when opening any page.
- **Quote Styling**: Decreased quote text line-height from 1.8 to 1.4.
- **Queue UI Fixes**: Prevented queue items from wrapping at narrow widths; removed overflow:hidden from riwayah checkboxes.
- **Juz Badge**: Reverted to 'Juz 30' format; removed juz_names font.
- **Version bumped**: `manifest.xml`, `version.json`, `updater.js`, `index.html` -> 2.5.5.

### Ornament Replacer - v2.0.0 -> v3.5.0
- **Google Drive Auto-Detect**: Copied `drive-scanner.js` with fs/path/os require fix.
- **Templates Support**: Auto-detect now also finds `mushafproject/templates/`.
...
### Symbol Palette - v1.0.0 -> v1.4.1
- **CEP Flags**: Added `--enable-nodejs` + `--mixed-context` to `manifest.xml`.
- **Google Drive Auto-Detect**: Full scanner using `window.DriveScanner`.
- **Nested Structure Support**: Auto-detect handles both flat and nested structures.
- **Self-Updater**: Added updater system.
- **Layer Tools**: Document layer scanner, colored layer buttons, move selection to layer.
- **Version bumped**: `manifest.xml`, `version.json` -> 1.4.1.
